...
import { updateProfile, updateEmail, reauthenticateWithCredential, deleteUser, updatePassword, EmailAuthProvider } from "firebase/auth";
...
export default () => {
    ...
    return (
        <div className={cx('profile')}>
            ...
            <span>Here you can edit public information about yourself.<br />
                If you signed in with Google or Facebook, you can't change your email and password.</span>
                ...
                <div className={cx('infor')}>
                    <div className={cx('email', 'group-input')}>
                        ...
                        <input
                            ...
                            type="text" value={currentEmail}
                            disabled={!isUpdateEmail ? true : false} />
                        {
                            !isUpdateEmail ? <button onClick={() => setUpdateEmail(true)}>Edit</button>
                                : <>
                                    <button onClick={handleSaveEmail}>Save</button>
                                    <button onClick={handleCancel(setUpdateEmail, setCurrentEmail, email)}>Cancel</button>
                                </>
                        }
                    </div>
                    <div className={cx('name', 'group-input')}>
                        ...
                        {
                            !isUpdateName ? <button onClick={() => setUpdateName(true)}>Edit</button>
                                : <>
                                    <button onClick={handleSaveName}>Save</button>
                                    <button onClick={handleCancel(setUpdateName, setCurrentName, displayName)}>Cancel</button>
                                </>
                        }
                    </div>
                    <div className={cx('password', 'group-input')}>
                        <h2>Change password</h2>
                        <input type="password" onChange={handleChange(setPassword)} placeholder='New password' />
                        <button onClick={handleSavePassword}>Change</button>
                    </div>
                    <div className={cx('delete-account')}>
                        <OutlineButton className={cx('small')} onClick={handleDeleteAccount}>Delete Account</OutlineButton>
                    </div>
                </div>
            ...
            <Modal active={modalActive} setActive={setModalActive} className={cx('cus-modal')}>
                <h2>Type your password again to reauthenticate</h2>
                <input type="password" value={oldPassword} onChange={handleChange(setOldPassword)} placeholder='Type your password' />
